...
function build_dist()
{
    ...
    local product=PRODUCT-aosp_${2}-${3}
    if [ "${QUICKBOOT}" == "true" ]; then
        cp -f ${TOP}/device/nexell/svt_ref/ota_from_target_files_svm.py ${TOP}/build/tools/releasetools/ota_from_target_files.py
        cp -f ${TOP}/device/nexell/svt_ref/releasetools_svm.py ${TOP}/device/nexell/svt_ref/releasetools.py
        make ${product} TARGET_SOC=${1} QUICKBOOT=1 QPART=1 dist -j8
    else
        cp -f ${TOP}/device/nexell/svt_ref/ota_from_target_files_normal.py ${TOP}/build/tools/releasetools/ota_from_target_files.py
        cp -f ${TOP}/device/nexell/svt_ref/releasetools_normal.py ${TOP}/device/nexell/svt_ref/releasetools.py
...
function post_process()
{
    ...
    if [ "${QUICKBOOT}" == "true" ] ; then
        ...
        if [ -d ${TOP}/device/nexell/svt_ref/svmdata ] ; then
			if [ -d ${result_dir}/svmdata ] ; then
				rm -rf ${result_dir}/svmdata
			fi
			cp -af ${TOP}/device/nexell/svt_ref/svmdata  ${result_dir}
...
function make_uboot_bootcmd()
{
	...
	if [ "${TARGET_SOC}" == "s5p4418" ]; then
		...
		local dtb_dest_addr=0x7A000000
		...
		local ramdisk_dest_addr=0x79000000
